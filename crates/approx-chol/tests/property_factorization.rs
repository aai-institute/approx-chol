...
fn ac2_config(backend: Backend) -> Config {
    Config {
        seed: 7,
        split_merge: Some(2),
        backend,
...
proptest! {
    ...
    fn default_factorization_solve_is_panic_free_and_finite(
        ...
        for backend in BACKENDS {
            ...
            prop_assert!(run.is_ok(), "{backend:?}: factorization or solve panicked");
            let work = run.or_panic("checked above");
            prop_assert!(work.iter().all(|x| x.is_finite()), "{backend:?}");
        }
    }
    ...
    fn default_solve_matches_solve_into(
        ...
        for backend in BACKENDS {
            ...
            for (a, b) in from_alloc.iter().zip(from_into.iter()) {
                prop_assert!((*a - *b).abs() <= 1e-12, "{backend:?}");
            }
        }
    }
    ...
    fn ac2_factorization_solve_is_panic_free_and_finite(
        ...
        for backend in BACKENDS {
            let run = catch_unwind(AssertUnwindSafe(|| {
                let csr = CsrRef::new(&row_ptrs, &col_indices, &values, n)
                    .or_panic("generated CSR must be valid");
                let factor = factorize_with(csr, ac2_config(backend))
            ...
            prop_assert!(run.is_ok(), "{backend:?}: AC2 factorization or solve panicked");
            let work = run.or_panic("checked above");
            prop_assert!(work.iter().all(|x| x.is_finite()), "{backend:?}");
        }
    }
    ...
    fn ac_residual_is_bounded(
        ...
        for backend in BACKENDS {
            ...
            prop_assert!(
                ...
                "{backend:?} relative residual too large: {:.4e} (r_norm={:.4e}, b_norm={:.4e})",
    ...
    fn ac2_residual_is_bounded(
        ...
        for backend in BACKENDS {
            ...
            prop_assert!(
                b_norm < 1e-15 || r_norm / b_norm < 100.0,
                "AC2 {backend:?} relative residual too large: {:.4e} (r_norm={:.4e}, b_norm={:.4e})",
                r_norm / b_norm, r_norm, b_norm
            );
        }
    }
    ...
    fn random_rhs_residual_is_bounded(
        ...
        for backend in BACKENDS {
            ...
            prop_assert!(
                r_norm / b_norm < 100.0,
                "{backend:?} random-RHS relative residual too large: {:.4e}", r_norm / b_norm
            );
        }
    }
    ...
    fn sddm_factorization_is_panic_free_and_finite(
        ...
        for backend in BACKENDS {
            ...
            prop_assert!(run.is_ok(), "{backend:?}: SDDM factorization or solve panicked");
            let x = run.or_panic("checked above");
            prop_assert!(x.iter().all(|v| v.is_finite()), "{backend:?}: non-finite values");
        }
    }
    ...
    fn deterministic_with_fixed_seed(
        ...
        for backend in BACKENDS {
            ...
            let csr1 = CsrRef::new(&row_ptrs, &col_indices, &values, n)
                .or_panic("valid CSR");
            let x1 = factorize_with(csr1, cfg).or_panic("factorize 1")
                .solve(&rhs).or_panic("solve 1");

            let csr2 = CsrRef::new(&row_ptrs, &col_indices, &values, n)
                .or_panic("valid CSR");
            let x2 = factorize_with(csr2, cfg).or_panic("factorize 2")
    ...
    fn sddm_factor_dimensions_are_consistent(
        ...
        for backend in BACKENDS {
            ...
            prop_assert!(
                factor.n() > n as usize,
                "{backend:?}: SDDM should trigger Gremban augmentation"
            );
        }
    }
